...
O segredo de assinatura vem de `LEGIS_RJ_SEGREDO_OAUTH`. Sem ele, cada partida do
...
class ProvedorOAuth(OAuthAuthorizationServerProvider):
    ...
    async def revoke_token(self, token: AccessToken | RefreshToken) -> None:
        ...
        troque `LEGIS_RJ_SEGREDO_OAUTH`: as assinaturas antigas param de valer.
        """
...
def montar(url_publica: str, segredo: str | None) -> tuple[ProvedorOAuth, AuthSettings]:
    ...
    if not segredo:
        ...
        log.warning(
            "LEGIS_RJ_SEGREDO_OAUTH não definido: usando um segredo temporário. "
